...
todo2.innerHTML = `Todo 2 Değişti 
    <a href="#" onclick="changeImg()" class="delete-item">
...
const todo22 = document.querySelectorAll(".list-group-item")[1];
...
clearB.addEventListener("click", clearF);
yakalama.addEventListener("click", denemeF);
console.log(todos);
...
function clearF() {
    document.querySelectorAll('.card-title')[0].textContent = 'Todo List Sayfası - Güncellenmiş hali';
}
...
// Üstteki ile benzer ancak window ile çağrılır.
window.addEventListener("load", run2);

// Üstüne basıldığında fonksiyonu çalıştırır.
const todoListesi = document.querySelectorAll(".card-title")[1];
todoListesi.addEventListener("click", run3);

// Double click yapıldığında çalışır.
todoListesi.addEventListener("dblclick", run4);

// ! mouseover -> Üstüne gelindiğinde çalışır
// Mouse üstünde durduğu sürece çalışmaya devam eder.
// ! mouseout -> İçine girdikten sonra çıkıldığında çalışır.
// ! mouseenter -> Üstüne gelindiğinde 1 kere çalışır.
// Mouse üstünde durduğu sürece sadece 1 kere çalışır.
// ! mouseleave -> mouseout gibi.

// Klavyeden bir tuşa (harf veya sayı) basıldığında çalışır.
document.addEventListener("keypress", run);

//! keydown -> keypress ile aynı ancak tüm tuşlarla çalışır.

function run(e) {

    // e.keycode veya e.which yazılırsa ASCII karşılığı verilir.
    console.log(e.key);
